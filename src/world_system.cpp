...
// ==================== BASIC FUNCTIONS ====================
// create the  world
WorldSystem::WorldSystem(SceneManager& manager) : Scene(manager)
	, player_health(0)
	, current_speed(1.0)
	, current_room({ 0, 0 }) {
...
void WorldSystem::init(RenderSystem* renderer_arg, GLFWwindow* window, AudioSystem* audio_arg) {
...
void WorldSystem::exit() {
	// needs to cleanup if exiting to menu, but freeze if exiting to any other state (like pause)
}

void WorldSystem::enter() {
	// needs to restart/load if entering from menu, but unfreeze if entering from any other state (like pause)
	// needs to know whether to load or to restart
}
...
void WorldSystem::update_music() {
	if (registry.players.get(player).combat_state == COMBAT_STATE::NO_COMBAT) {
		audio_system->set_music_volume(8);
		audio_system->play_music(SONG_ASSET_ID::POST_COMBAT_MUSIC);
	}
	else if (registry.players.get(player).combat_state == COMBAT_STATE::NORMAL_COMBAT) {
		audio_system->set_music_volume(16);
		audio_system->play_music(SONG_ASSET_ID::COMBAT_MUSIC);
	}
	else if (registry.players.get(player).combat_state == COMBAT_STATE::BOSS_ONE_COMBAT) {
		audio_system->set_music_volume(16);
		audio_system->play_music(SONG_ASSET_ID::BOSS_ONE_MUSIC);
	}
	else if (registry.players.get(player).combat_state == COMBAT_STATE::BOSS_TWO_COMBAT) {
		audio_system->set_music_volume(16);
		audio_system->play_music(SONG_ASSET_ID::BOSS_TWO_MUSIC);
	}
	else if (registry.players.get(player).combat_state == COMBAT_STATE::BOSS_THREE_COMBAT) {
		audio_system->set_music_volume(16);
		audio_system->play_music(SONG_ASSET_ID::BOSS_THREE_MUSIC);
	}
}
		...
		if (registry.players.components[0].combat_state == COMBAT_STATE::NO_COMBAT) {
			...
			case TEXTURE_ASSET_ID::DOOR_UP_DOWN_CLOSED:
				if (usedtext != TEXTURE_ASSET_ID::DOOR_UP_DOWN) {
					...
					audio_system->play_sound(SOUND_ASSET_ID::DOOR_CHANGE_SOUND, -1);
				}
				break;
	...
	if (registry.players.has(entity)) {
		if (left_mouse_button && !registry.players.get(player).dead) {
			...
			if (elapsed_ms >= new_fire_rate) {
				...
				audio_system->play_sound(SOUND_ASSET_ID::PLAYER_SHOOTING_SOUND, -1);
		...
		if ((elapsed_ms >= registry.shooters.get(entity).fire_rate)) {
			if (registry.players.get(player).combat_state == COMBAT_STATE::BOSS_ONE_COMBAT) {
				...
				audio_system->play_sound(SOUND_ASSET_ID::ENEMY_SHOOTING_SOUND, -1);
				set_last_shot_time(entity);
			}
		}
	}
